...
The system SHALL let any visitor download a single JSON file containing every book, copy, reading event, reader, shelf, wishlist item, and loan currently in the library, with no sign-in required.

#### Scenario: Downloading a backup

- **WHEN** a visitor clicks the backup download action on `/ajustes`
- **THEN** a JSON file downloads containing `books`, `copies`, `readingEvents`, `readers`, `shelves`, `wishlistItems`, and `loans` arrays reflecting the library's current data
