...
- *Why flat config:* greenfield on ESLint 9; the legacy `.eslintrc` path is
  deprecated. *Alternative:* `next lint` only — rejected, it doesn't cover the Express
...
- *Why:* a single global set would mis-flag DOM in node files and vice-versa, and
...
- *Why Vitest over Jest:* native ESM/TS (matches `tsx`, `"type"` ESM-ish, zod v4),
  fast, first-class jsdom + RTL. Jest needs ts-jest/babel ESM gymnastics. *Why two
  projects:* don't pay jsdom cost for server tests; keep environments honest.
...
- *Why:* the issue asks for an "example" test; we make it genuine coverage of the
...
- *Why no typecheck in the hook:* `tsc` over the project is too slow for a commit
...
- *Why one fast job:* it must be quick and reliable so it can be a required gate; the
